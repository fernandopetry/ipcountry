...
$outher = json_decode($json,true);
//var_dump(json_decode($json,true));

//ABRE O ARQUIVO TXT
$ponteiro = fopen("list.txt", "r");
...
//    $part = explode(" ", $linha);
//    var_dump($part);
    $sigla = substr($linha, 0,5);
    $siglas = explode("-",$sigla);
    
    $paiss = explode($sigla, $linha);
    
    $d['pais'] = trim($paiss[1]);
    
    $d['extensao'] = '.'.strtolower($siglas[1]);
    
    $d['locale_'] = str_replace('-', '_', substr($linha, 0,5));
    $d['locale'] = substr($linha, 0,5);
    $d['sigla2'] = $siglas[1];
//    $dd = \Petry\IpCountry\Data::getCounty($d['extensao']);
    /**
      array (size=5)
      'ordem' => int 3
      'nome' => string 'Albânia' (length=8)
      'sigla2' => string 'AL' (length=2)
      'sigla3' => string 'ALB' (length=3)
      'codigo' => string '008' (length=3)
     */
    foreach ($outher as $t) {
        if($t['sigla2']==$d['sigla2']){
            $d['sigla3'] = $t['sigla3'];
            $d['ibge'] = (int)$t['codigo'];
        }
    }
...
//    var_dump($d);
    ...
    echo '$data["'.$d['extensao'].'"]=["extensao"=>"'.$d['extensao'].'","pais"=>"'.$d['pais'].'","locale_"=>"'.$d['locale_'].'","locale"=>"'.$d['locale'].'","sigla2"=>"'.$d['sigla2'].'","sigla3"=>"'.$d['sigla3'].'","ibge"=>'.$d['ibge'].'];'.PHP_EOL;
    
    /**
       'pais' => string 'Kuwait' (length=6)
  'extensao' => string '.kw' (length=3)
  'locale_' => string 'ar_KW' (length=5)
  'locale' => string 'ar-KW' (length=5)
  'sigla2' => string 'KW' (length=2)
  'sigla3' => string 'KWT' (length=3)
  'ibge' => int 414
     */
